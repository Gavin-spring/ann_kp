...
def run_benchmarks():
    ...
    if not test_files:
        logger.error(f"Error: No test cases found in '{TEST_SUITE_DIR}'.")
    ...
    for filepath in sorted(test_files, key=lambda p: int(p.split('_n')[1].split('_')[0])):
        ...
        for name, func in ALGORITHMS_TO_TEST.items():
            ...
            logger.debug(f"  -> {name}: {execution_time:.6f} seconds")
...
def plot_results(df):
    ...
    output_path = os.path.join(PLOT_DIR, "benchmark_comparison.png") #TODO: replace by pdf after testing
